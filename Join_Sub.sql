...
-- 만약 기준 테이블에 조건이 일치하는 레코드가 존재하지 않으면 null로 표현
...
-- 만약 기준 테이블 혹은 조합할 테이블의 조건에 일치하는 레코드가 존재하지 않으면 null로 반환
...
-- 서브 쿼리를 FROM 절에서 사용할 땐 반드시 별칭을 사용해야 함
SELECT *
FROM (
	SELECT * FROM department
) D
